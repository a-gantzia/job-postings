...
from nlp_salary_estimator import predict_salary_range_desc
...
with st.sidebar:
    choose = option_menu(
        ...
        icons=["house", "app-indicator", "file-slides", "person lines fill"],
        menu_icon="list",
        default_index=0,
        styles={
            "container": {"padding": "5!important", "background-color": "#fffff"},
            "icon": {"color": "#3385ff", "font-size": "25px"},
            "nav-link": {
                "font-size": "16px",
                "text-align": "left",
                "margin": "0px",
...
if choose == "About":
    ...
    with col1:
        ...
        st.markdown(
            '<p class="font">Your Personalized Path to a Fair Salary</p>',
    ...
    st.write(
        "We are using",
        len(df),
        "different US tech job positions between 2022 and 2023, which have been used to train our Salary Estimator.",
    )
    st.write(
        """Are you navigating the tech job market? Step into a future where you're empowered with knowledge and insight. Our cutting-edge demo is not just a tool; it's your personal guide to the tech industry's salary landscape.
...
- Stay Ahead of the Curve: The tech industry evolves rapidly. So does our data. Keep pace with the latest salary trends and job positions to ensure you're always one step ahead."""
    ...
    selected_job = st.selectbox("Select your ideal job position", jobs, index=0).lower()
    if selected_job:
        salary_COL = salary_COL[salary_COL.job_category == selected_job]
        df = df[df.job_category == selected_job]
        st.write(
            "According to",
            (sum(salary_COL.observations)),
            """different US tech job positions between in this occupation, we provide an analysis of the Skills you need and the  expectations you should have
            according to the US State you could work from (accounting also for cost of living).""",
        )

        st.markdown(style2, unsafe_allow_html=True)
        st.markdown(
            '<p class="font">Salary and Cost of Living Expectations</p>',
            unsafe_allow_html=True,
        )
        ...
        fig5 = plot_maker.makingclouds(
            df,
            "clean_job_description_filtered",
            ppl_standing_img,
            "wordcloud_ppl_standing",
        )
        plt.axis("off")
        st.pyplot()
    ...
    with col1:
        input_description = st.text_input("Enter job description:")
        API_KEY = st.text_input("Enter your HuggingFace API key", type="password")
    with col2:
        pred_button = st.button("Predict Salary")
    if pred_button:
        predicted_range = predict_salary_range_desc(input_description, API_KEY)
        st.markdown(style1, unsafe_allow_html=True)
    ...
    input_data = {}
    input_data["skills"] = st.multiselect(
        "Skills", skills_final, placeholder="Select your skills"
    )
    input_data["prog_lang"] = st.multiselect(
    ...
    input_data["seniority"] = st.selectbox(
        "Select seniority level", ["Junior", "Mid", "Senior"], index=0
    )
    categories = [
        "location",
        "company_type",
    ...
    selected_areas = st.text_input(
        "Input two areas of interest where you would like to work (comma-separated)"
    )
    selected_areas = [area.strip() for area in selected_areas.split(",")]

    postings = pd.read_csv("data/clean_job_postings_w_salary.csv").drop(
        columns=["Unnamed: 0"]
    )
    ...
    st.header(
        "Here is a table of recommended jobs that you should check out based on your skills and areas of interest:"
    )
    st.dataframe(df)
